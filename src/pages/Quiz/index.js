...
export default function Quiz({ navigation }) {
  ...
  const response = [
    [{
      id: 1,
      question: 'A alternativa que pareia corretamente o material com a cor do cesto é:',
      options: [
    ...
    [{
      id: 3,
      question: 'Assinale a alternativa que aponta corretamente os objetos que NÃO podem ser reciclados.',
      options: [
    ...
    [{
      id: 4,
      question: 'Qual é o cuidado a ser tomado antes de enviar um determinado material para a reciclagem?',
      options: [
    ...
    [{
      id: 5,
      question: 'Quais são as consequências mais diretas do despejo inadequado do lixo?',
      options: [
        'Enchentes e doenças.',
    ...
    [{
      id: 10,
      question: 'Quais dos materiais abaixo podem gerar algum tipo de combustível?',
      options: [
        'Óleo e metal.',
        'Papel e plástico.',
      ...
      <Container>
        <Title>Quiz</Title>
        {
          currentQuestion <= len || len === 1 ?
            <Form>
              <Question>
                <Text>Pergunta: {currentQuestion} de {len}</Text>
              ...
              <FlatList
                ...
                renderItem={({ item }) => <Answer check={answerCheckd} onPress={() => handleAnswer(item)}>{item}</Answer>}
              />
